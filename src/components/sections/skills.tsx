 ...
 */
/** Remote icon with a text-mark fallback if a logo fails to load. */
const SkillIcon = ({ skill }: { skill: Skill }) => {
  ...
  if (failed || !skill.icon) {
    return (
      <span
        aria-hidden
        className="relative flex size-9 items-center justify-center rounded-md border border-border/60 text-[11px] font-bold tracking-tight text-foreground/70 md:size-11 md:text-xs"
      >
        {skill.fallback}
      </span>
    );
